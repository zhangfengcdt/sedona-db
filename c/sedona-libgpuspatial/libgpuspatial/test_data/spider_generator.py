...
class WKTSinkInMemory(DataSink):
    def __init__(self, output: list):
        self.output = output

    def writePoint(self, coordinates):
        s = "POINT("
        s += " ".join([str(elem) for elem in coordinates])
        s += ")"
        self.output.append(s)

    def writeBox(self, minCoordinates, maxCoordinates):
        s = "POLYGON(("
        s += f"{minCoordinates[0]} {minCoordinates[1]},"
        s += f"{maxCoordinates[0]} {minCoordinates[1]},"
        s += f"{maxCoordinates[0]} {maxCoordinates[1]},"
        s += f"{minCoordinates[0]} {maxCoordinates[1]},"
        s += f"{minCoordinates[0]} {minCoordinates[1]}"
        s += "))"
        self.output.append(s)
    def writePolygon(self, coordinates):
        s = "POLYGON(("
        for coord in coordinates:
            s += " ".join([str(elem) for elem in coord])
            s += ","
        s += f"{coordinates[0][0]} {coordinates[0][1]}"
        s += "))"
        self.output.append(s)
