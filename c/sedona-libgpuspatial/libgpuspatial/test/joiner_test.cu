...
TEST(JoinerTest, PIP) {
  ...
  for (int i = 0; i < n_row_groups; i++) {
    ...
    spatial_joiner.PushStream(context.get(), nullptr, stream_array.get(), 0, stream_array->length,
                             Predicate::kContains, &build_indices, &array_indices,
                             array_index_offset);
      ...
      GEOSSTRtree_query_r(
          ...
          [](void* item, void* data) {
            ...
            if (GEOSContains_r(payload->handle, polygon1, polygon2) == 1) {
              auto polygon1_id = (size_t)GEOSGeom_getUserData_r(payload->handle, polygon1);
              auto polygon2_id = (size_t)GEOSGeom_getUserData_r(payload->handle, polygon2);
              payload->build_indices.push_back(payload->build_index_offset + polygon1_id);
              payload->stream_indices.push_back(payload->stream_index_offset + polygon2_id);
            }
          },
          (void*)&payload);
    }
