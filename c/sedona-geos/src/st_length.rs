...
fn invoke_scalar(geos_geom: &geos::Geometry) -> Result<f64> {
    ...
    match geos_geom.geometry_type() {
        LineString => Ok(geos_geom.length().unwrap()),
        MultiLineString => Ok(geos_geom.length().unwrap()),
        GeometryCollection => Ok(geos_geom.length().unwrap()),
        _ => Ok(0.0),
    }
}
...
mod tests {
    ...
    fn udf(#[values(WKB_GEOMETRY, WKB_VIEW_GEOMETRY)] sedona_type: SedonaType) {
        ...
        let input_wkt = vec![
            ...
            Some("GEOMETRYCOLLECTION (POINT (0 0), LINESTRING (0 0, 1 0, 0 1), LINESTRING (0 0, 1 0, 0 1), POINT (0 0))"),
        ];
